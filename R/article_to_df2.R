...
{
  options(warn = -1)
  if (class(pubmedArticle) != "character" | regexpr("(<PubmedArticle)(.+)(\\/PubmedArticle>)",
                                                    pubmedArticle) < 0) {
  ...
  tryCatch({
    tmp.article <- easyPubMed::custom_grep(xml_data = pubmedArticle,
                               tag = "PubmedArticle", format = "char")
    if (is.null(tmp.article)) {
      message("An error occurred")
      return(NULL)
    }
    tmp.title <- easyPubMed::custom_grep(xml_data = tmp.article, tag = "ArticleTitle",
                             format = "char")
    ...
    tmp.abstract <- easyPubMed::custom_grep(xml_data = tmp.article,
    ...
    if (length(sel.dateType) < 1) {
      ...
      tmp.date <- easyPubMed::custom_grep(xml_data = tmp.article,
                              tag = my.dateType[sel.dateType], format = "char")
      tmp.date <- sapply(c("Year", "Month", "Day"), (function(tt) {
        tdat.el <- easyPubMed::custom_grep(xml_data = tmp.date,
                               tag = tt, format = "char")
        ifelse(is.null(tdat.el), NA, tdat.el[1])
      }))
    }
    tmp.paperID <- easyPubMed::custom_grep(xml_data = tmp.article, tag = "ArticleIdList",
                               format = "char")
    ...
    tmp.jabbrv <- easyPubMed::custom_grep(xml_data = tmp.article, tag = "ISOAbbreviation",
                              format = "char")
    tmp.jabbrv <- ifelse(is.null(tmp.jabbrv), NA, tmp.jabbrv)
    tmp.journal <- easyPubMed::custom_grep(xml_data = tmp.article, tag = "Title",
                               format = "char")
    tmp.journal <- ifelse(is.null(tmp.journal), NA, tmp.journal)
    tmp.keys <- tryCatch({
      if (getKeywords) {
        tmp.keys <- easyPubMed::custom_grep(xml_data = tmp.article,
                                tag = "Keyword", format = "char")
        tmp.mesh <- easyPubMed::custom_grep(xml_data = tmp.article,
                                tag = "MeshHeading", format = "char")
        if (length(tmp.mesh) > 0) {
          tmp.mesh <- sapply(tmp.mesh, function(xxm) {
            easyPubMed::custom_grep(xml_data = xxm, tag = "DescriptorName",
                        format = "char")
          })
        }
    ...
    tmp.authors <- easyPubMed::custom_grep(xml_data = tmp.article, tag = "AuthorList",
                               format = "char")
    if (length(tmp.authors) < 1 | !getAuthors) {
      ...
      author.list <- easyPubMed::custom_grep(xml_data = tmp.authors,
                                 tag = "Author", format = "char")
      final.mat <- do.call(rbind, lapply(author.list,
                                         (function(al) {
                                           tmp.lastnm <- easyPubMed::custom_grep(xml_data = al, tag = "LastName",
                                                                     format = "char")
                                           tmp.firstnm <- easyPubMed::custom_grep(xml_data = al,
